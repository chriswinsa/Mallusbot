...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("😁 𝗛𝗲𝘆 𝗙𝗿𝗶𝗲𝗻𝗱,𝗣𝗹𝗲𝗮𝘀𝗲 𝗦𝗲𝗮𝗿𝗰𝗵 𝗬𝗼𝘂𝗿𝘀𝗲𝗹𝗳 നീ ഫയൽ റിക്വസ്റ്റ് ചെയ് ബ്രോ .", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('💠 GᏒԾUᎮ 💠', url='https://t.me/+sYJxSG9IcYM5NTNh'),
            InlineKeyboardButton('💠 OWneR 💠', url='https://t.me/filesharingnewbot')
            ],[
        ...
        buttons= [[
            InlineKeyboardButton('♥️ SԾUᏒCΞ ♥️', url='https://t.me/zee_kerala_movie_group')
            ],[
            InlineKeyboardButton('🏠 𝗛𝗼𝗺𝗲 🏠', callback_data='start'),
...
async def advantage_spell_chok(msg):
    ...
    await msg.reply("𝙲𝙷𝙴𝙲𝙺 𝚈𝙾𝚄𝚁 𝙼𝙾𝚅𝙸𝙴 𝙾𝙽 𝚃𝙷𝙴 𝙶𝙸𝚅𝙴𝙽 𝙻𝙸𝚂𝚃 𝙰𝙽𝙳 𝚂𝙴𝙻𝙴𝙲𝚃 𝚈𝙾𝚄𝚁 𝙼𝙾𝚅𝙸𝙴𝚂.📌 set ALLE 🥰", reply_markup=InlineKeyboardMarkup(btn))
